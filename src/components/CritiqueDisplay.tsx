...
export default function CritiqueDisplay({ critique, trackInfo, onClear, localFileBlobUrl, onViewDefinition, onOpenArConsult, onNavigateToRabbitHole, onNavigateToEngineeringStudio }: CritiqueDisplayProps) {
  ...
  const [isMixBalanceExpanded, setIsMixBalanceExpanded] = useState(false);
  const [isVocalTrackingExpanded, setIsVocalTrackingExpanded] = useState(false);
  const liveMetrics = critique?.liveMetrics;
  ...
  return (
    <div className="flex flex-col gap-6" id="critique-display-container">
      ...
      {expandedCategory === "sonic" && (
        ...
        {/* Card: Mix Balance Quality (indigo theme) */}
        <div className="flex flex-col w-full gap-4 mt-6" id="sidebar-link-sonic-4">
          <button
            onClick={() => setIsMixBalanceExpanded(!isMixBalanceExpanded)}
            className={`relative z-10 flex flex-col justify-between py-[15px] px-6 h-[180px] rounded-[24px] border transition-all duration-300 text-left cursor-pointer group overflow-hidden select-none text-white w-full ${
              isMixBalanceExpanded
                ? "bg-[#090b0e] border-indigo-500 shadow-[0_0_35px_rgba(99,102,241,0.35)] ring-1 ring-indigo-500/40 font-black"
                : "bg-[#0A0B0E]/60 border-indigo-500/60 hover:border-indigo-500 hover:bg-neutral-900/40 text-slate-400"
            }`}
          >
            {isMixBalanceExpanded ? (
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-950/10 via-neutral-950 to-[#03050a] pointer-events-none" />
            ) : (
              <div className="absolute inset-0 bg-gradient-to-br from-neutral-950 to-[#03050a] pointer-events-none" />
            )}
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6 w-full h-full">
                ...
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl border flex-shrink-0 flex items-center justify-center transition-all ${
                    isMixBalanceExpanded
                      ...
                      : "bg-neutral-900 border-white/5 text-slate-500 group-hover:text-indigo-400"
                  }`}>
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`font-black text-[19px] tracking-wider uppercase transition-colors ${
                        isMixBalanceExpanded ? "text-white" : "text-slate-400 group-hover:text-slate-200"
                      }`}
                    >
                      MIX BALANCE QUALITY
                    </span>
                    <span className="text-[10px] text-slate-500 font-medium">Stereo Soundstage Balance</span>
                  </div>
                </div>

                <div className={`border-t text-left pt-2 px-0.5 transition-colors ${
                  isMixBalanceExpanded ? "border-indigo-500/15" : "border-white/5"
                }`}>
                  <p className="text-[10px] text-slate-400 leading-relaxed font-semibold mb-2">
                    Audits master frequency balances, low-end boominess, midrange vocal presence, and stereo width separation.
                    <span className="block mt-1 text-indigo-400/90 font-mono text-[8.5px] uppercase tracking-wider">DRIVES 45% OF YOUR SONIC SOUNDPRINT SCORE.</span>
                  </p>
                  <span className={`inline-block text-[9px] font-mono tracking-widest px-2 py-0.5 rounded-full border transition-all ${
                    isMixBalanceExpanded
                      ? "bg-indigo-500/10 border-indigo-500/20 text-indigo-400"
                      : "bg-neutral-900/50 border-white/5 text-slate-600 group-hover:text-indigo-400"
                  }`}>
                    {isMixBalanceExpanded ? "ACTIVE ⬇" : "VIEW METRICS ⚡"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-shrink-0 flex items-center justify-center">
                <ScoreCircle
                  ...
                  size={110}
                  strokeWidth={7}
                  color={isMixBalanceExpanded ? "#6366f1" : "rgba(99, 102, 241, 0.45)"}
                  glowColor={isMixBalanceExpanded ? "rgba(99, 102, 241, 0.65)" : "rgba(99, 102, 241, 0.15)"}
                  extraGlow={isMixBalanceExpanded}
                />
              </div>
            </div>
          </button>
          ...
          <AnimatePresence initial={false}>
            {isMixBalanceExpanded && (
              <motion.div
                ...
                className="overflow-hidden w-full relative z-0"
              >
                <div style={{ position: "relative", left: "15px", width: "calc(100% - 15px)" }} className="bg-[#0A0B0E] border border-indigo-500 rounded-3xl p-6 shadow-[0_0_35px_rgba(0,0,0,0.95)] flex flex-col gap-5">
                  <div style={{ fontFamily: "Inter, sans-serif", fontWeight: "bold", color: "#ffffff", fontSize: "16px" }}>
                    MIX BALANCE QUALITY AUDIT
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {[
                      { name: "Mud Prevention", weight: "25%", desc: "Evaluation of the 150Hz–250Hz proximity effect zone to ensure vocal and snare clarity." },
                      { name: "Midrange Spacing", weight: "25%", desc: "Ensuring guitars and synths do not drown out vocal track focus." },
                      { name: "Low-End Division", weight: "20%", desc: "Frequency splitting between sub-bass movements and kick-drum transient punch." },
                      { name: "Sibilance Shaving", weight: "15%", desc: "Auditing the sibilance region (4kHz-8kHz) for harsh sibilant spikes." },
                      { name: "Stereo Width", weight: "15%", desc: "Judges whether the stereo image is appropriately wide without sacrificing mono compatibility or center-focus." }
                    ].map((sub, idx) => (
                      <div key={idx} className="p-4 rounded-xl border bg-[#0d0d14] border-indigo-500/15">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-300">{sub.name}</span>
                          <span className="text-[9px] font-mono font-bold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded-full">{sub.weight}</span>
                        </div>
                  ...
                  <div className="p-4 bg-[#020203] border border-white/10 rounded-xl">
                    <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-slate-500 block mb-1.5">Live Readout</span>
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      {critique?.mixQuality?.stereoField ?? "Solid width."} Low frequencies: {critique?.mixQuality?.frequencyBalance?.lowEnd ?? "Balanced"}. Midrange: {critique?.mixQuality?.frequencyBalance?.midrange ?? "Clear"}. High-end sibilance: {critique?.mixQuality?.frequencyBalance?.highEnd ?? "Contained"}. {critique?.mixQuality?.dominanceIssues ?? "No bloating present."}
                    </p>
                  </div>

                  <p className="text-[10px] text-slate-500 leading-relaxed border-t border-white/5 pt-3">
                    Mix Balance Quality is weighted at 45% into your overall Sonic Soundprint score — the single largest ingredient in that category.
                  </p>
                </div>
              </motion.div>
            )}
        ...
        {/* Card: Vocal Tracking (purple theme) */}
        <div className="flex flex-col w-full gap-4 mt-6" id="sidebar-link-sonic-5">
          <button
            onClick={() => setIsVocalTrackingExpanded(!isVocalTrackingExpanded)}
            className={`relative z-10 flex flex-col justify-between py-[15px] px-6 h-[180px] rounded-[24px] border transition-all duration-300 text-left cursor-pointer group overflow-hidden select-none text-white w-full ${
              isVocalTrackingExpanded
                ? "bg-[#090b0e] border-purple-500 shadow-[0_0_35px_rgba(168,85,247,0.35)] ring-1 ring-purple-500/40 font-black"
                : "bg-[#0A0B0E]/60 border-purple-500/60 hover:border-purple-500 hover:bg-neutral-900/40 text-slate-400"
            }`}
          >
            {isVocalTrackingExpanded ? (
              <div className="absolute inset-0 bg-gradient-to-br from-purple-950/10 via-neutral-950 to-[#03050a] pointer-events-none" />
            ) : (
              <div className="absolute inset-0 bg-gradient-to-br from-neutral-950 to-[#03050a] pointer-events-none" />
            )}
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6 w-full h-full">
              <div className="flex flex-col flex-1 justify-between gap-3 h-full">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-xl border flex-shrink-0 flex items-center justify-center transition-all ${
                    isVocalTrackingExpanded
                      ? "bg-purple-500/10 border-purple-500/30 text-purple-400 shadow-[0_0_15px_rgba(168,85,247,0.3)]"
                      : "bg-neutral-900 border-white/5 text-slate-500 group-hover:text-purple-400"
                  }`}>
                  ...
                  <div className="flex flex-col">
                    <span
                      className={`font-black text-[19px] tracking-wider uppercase transition-colors ${
                        isVocalTrackingExpanded ? "text-white" : "text-slate-400 group-hover:text-slate-200"
                      }`}
                    >
                      VOCAL TRACKING
                    </span>
                    <span className="text-[10px] text-slate-500 font-medium">Vocal Chain Engineering</span>
                  </div>
                </div>

                <div className={`border-t text-left pt-2 px-0.5 transition-colors ${
                  isVocalTrackingExpanded ? "border-purple-500/15" : "border-white/5"
                }`}>
                  <p className="text-[10px] text-slate-400 leading-relaxed font-semibold mb-2">
                    Measures vocal delivery accuracy, pitch consistency, breath management, dynamic levels, and vocal chain space.
                    <span className="block mt-1 text-purple-400/90 font-mono text-[8.5px] uppercase tracking-wider">DRIVES 30% OF YOUR SONIC SOUNDPRINT SCORE.</span>
                  </p>
                  <span className={`inline-block text-[9px] font-mono tracking-widest px-2 py-0.5 rounded-full border transition-all ${
                    isVocalTrackingExpanded
                      ? "bg-purple-500/10 border-purple-500/20 text-purple-400"
                      : "bg-neutral-900/50 border-white/5 text-slate-600 group-hover:text-purple-400"
                  }`}>
                    {isVocalTrackingExpanded ? "ACTIVE ⬇" : "VIEW METRICS ⚡"}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-shrink-0 flex items-center justify-center">
                <ScoreCircle
                  ...
                  size={110}
                  strokeWidth={7}
                  color={isVocalTrackingExpanded ? "#a855f7" : "rgba(168, 85, 247, 0.45)"}
                  glowColor={isVocalTrackingExpanded ? "rgba(168, 85, 247, 0.65)" : "rgba(168, 85, 247, 0.15)"}
                  extraGlow={isVocalTrackingExpanded}
                />
              </div>
            </div>
          </button>
          ...
          <AnimatePresence initial={false}>
            {isVocalTrackingExpanded && (
              <motion.div
                ...
                className="overflow-hidden w-full relative z-0"
              >
                <div style={{ position: "relative", left: "15px", width: "calc(100% - 15px)" }} className="bg-[#0A0B0E] border border-purple-500 rounded-3xl p-6 shadow-[0_0_35px_rgba(0,0,0,0.95)] flex flex-col gap-5">
                  <div style={{ fontFamily: "Inter, sans-serif", fontWeight: "bold", color: "#ffffff", fontSize: "16px" }}>
                    VOCAL TRACKING AUDIT
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {[
                      { name: "Pitch Accuracy", weight: "40%", desc: "Steady tonality, correct note targeting, and professional pitch scale alignment." },
                      { name: "Dynamic Delivery", weight: "35%", desc: "Vocal compression tightness, breath control consistency, and lyrical phrasing power." },
                      { name: "Vocal Layer Fit", weight: "25%", desc: "Vocal depth placement, backing doubles, tuning effects, and reverb sends." }
                    ].map((sub, idx) => (
                      <div key={idx} className="p-4 rounded-xl border bg-[#14101a] border-purple-500/15">
                        <div className="flex items-center justify-between mb-1.5">
                          <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-300">{sub.name}</span>
                          <span className="text-[9px] font-mono font-bold text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-full">{sub.weight}</span>
                        </div>
                        <p className="text-[10px] text-slate-400 leading-relaxed">{sub.desc}</p>
                      </div>
                    ))}
                  ...
                  <div className="p-4 bg-[#020203] border border-white/10 rounded-xl">
                    <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-slate-500 block mb-1.5">Live Readout</span>
                    <p className="text-[11px] text-slate-300 leading-relaxed">
                      {critique?.performance?.vocalsCritique ?? "Lead tracks are balanced and fit the target layout."}
                    </p>
                  </div>

                  <p className="text-[10px] text-slate-500 leading-relaxed border-t border-white/5 pt-3">
                    Vocal Tracking is weighted at 30% into your overall Sonic Soundprint score.
                  </p>
                </div>
              </motion.div>
            )}
