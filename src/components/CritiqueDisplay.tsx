...
export default function CritiqueDisplay({ critique, trackInfo, onClear, localFileBlobUrl, onViewDefinition, onOpenArConsult, onNavigateToRabbitHole, onNavigateToEngineeringStudio }: CritiqueDisplayProps) {
  ...
  // --- V2 TESTING: real, computed Chord Dynamics score from actual detected chord data ---
  // (critique.liveMetrics.detectedChordProgressionNamed), instead of folding it opaquely
  // into Gemini's single holistic Music Theory score.
  const v2ChordData = liveMetrics?.detectedChordProgressionNamed ?? [];
  let v2ChordDynamicsScore: number | null = null;
  let v2ChordDynamicsDetail = "Not enough detected chord data to compute a real score yet.";
  if (v2ChordData.length >= 2) {
    const uniqueChords = new Set(v2ChordData.map((c: any) => `${c.root}-${c.quality}`)).size;
    const chordVocabScore = Math.min(100, (uniqueChords / 4) * 100);

    let transitions = 0;
    let movementCount = 0;
    let strongMovementCount = 0;
    for (let i = 1; i < v2ChordData.length; i++) {
      const prevRoot = v2ChordData[i - 1].root;
      const currRoot = v2ChordData[i].root;
      if (prevRoot === currRoot && v2ChordData[i - 1].quality === v2ChordData[i].quality) continue; // skip held/repeated chord, not a real transition
      transitions++;
      if (prevRoot !== currRoot) movementCount++;
      const rawDiff = Math.abs(currRoot - prevRoot) % 12;
      const interval = Math.min(rawDiff, 12 - rawDiff);
      if (interval === 5 || interval === 7) strongMovementCount++; // perfect 4th/5th — circle-of-fifths functional motion
    }
    const motionScore = transitions > 0
      ? ((movementCount / transitions) * 60) + ((strongMovementCount / transitions) * 40)
      : 50;

    v2ChordDynamicsScore = Math.round((chordVocabScore * 0.5) + (motionScore * 0.5));
    v2ChordDynamicsDetail = `${uniqueChords} unique chord${uniqueChords === 1 ? "" : "s"} detected across ${v2ChordData.length} segments. ${strongMovementCount} of ${transitions || 0} real transitions used perfect 4th/5th (circle-of-fifths) root motion.`;
  }

  // --- V2 TESTING: real, computed Melody score from actual transcribed note data ---
  // (critique.liveMetrics.detectedMelodyNotes), instead of folding it opaquely into
  ...
  if (v2MelodyData && v2MelodyData.notes && v2MelodyData.notes.length >= 30) {
    ...
    // Healthy stepwise-to-leap balance for singable, well-contoured melodic writing sits
    // roughly between 50%-80% stepwise motion — not purely scalar, not too disjointed.
    let contourScore: number;
    if (stepRatio >= 0.5 && stepRatio <= 0.8) {
      contourScore = 100;
    } else if (stepRatio < 0.5) {
      contourScore = Math.max(0, Math.round((stepRatio / 0.5) * 100));
    } else {
      contourScore = Math.max(0, Math.round(100 - ((stepRatio - 0.8) / 0.2) * 100));
    }

    const totalEvents = totalSteps + totalLeaps + totalRepeats;
    const repeatRatio = totalEvents > 0 ? totalRepeats / totalEvents : 0;
    // A little repetition aids memorability (hooks); too much reads as static/boring.
    let repeatScore: number;
    if (repeatRatio >= 0.05 && repeatRatio <= 0.25) {
      repeatScore = 100;
    } else if (repeatRatio < 0.05) {
      repeatScore = Math.max(0, Math.round((repeatRatio / 0.05) * 100));
    } else {
      repeatScore = Math.max(0, Math.round(100 - ((repeatRatio - 0.25) / 0.35) * 100));
    }

    v2MelodyScore = Math.round((contourScore * 0.65) + (repeatScore * 0.35));
    v2MelodyDetail = `Based on ${v2MelodyData.notes.length} detected pitch events: ${Math.round(stepRatio * 100)}% stepwise motion, ${totalRepeats} repeated notes. Sample size affects confidence — very low counts should be read cautiously.`;
  }
  ...
  return (
    <div className="flex flex-col gap-6" id="critique-display-container">
      ...
      {expandedCategory === "sonic" && (
        ...
        {/* Card: Music Theory Analysis V2 TESTING ELEMENT — duplicate using real computed BT data where available */}
        <div className="flex flex-col w-full gap-4" id="sidebar-link-compositional-3-v2">
          {(() => {
            const isExpanded = expandedMetric === "theory-v2";
            const v2FormScore = theoryScore; // no real Structural Boundary Detection data yet — still Gemini-estimated
            const v2HasFullData = v2ChordDynamicsScore !== null && v2MelodyScore !== null;
            const v2OverallScore = v2HasFullData
              ? Math.round((v2ChordDynamicsScore! * 0.40) + (v2MelodyScore! * 0.30) + (v2FormScore * 0.30))
              : null;
            return (
              <>
                <button
                  onClick={() => setExpandedMetric(isExpanded ? null : "theory-v2")}
                  className={`relative z-10 flex flex-col justify-between py-[15px] px-6 h-[159px] rounded-[24px] border transition-all duration-300 text-left cursor-pointer group overflow-hidden select-none text-white w-full ${
                    isExpanded
                      ? "bg-[#090b0e] border-orange-500 shadow-[0_0_35px_rgba(249,115,22,0.35)] ring-1 ring-orange-500/40 font-black"
                      : "bg-[#0A0B0E]/60 border-orange-500/40 hover:border-orange-500 hover:bg-neutral-900/40 text-slate-400"
                  }`}
                >
                  {isExpanded ? (
                    <div className="absolute inset-0 bg-gradient-to-br from-orange-950/10 via-neutral-950 to-[#03050a] pointer-events-none" />
                  ) : (
                    <div className="absolute inset-0 bg-gradient-to-br from-neutral-950 to-[#03050a] pointer-events-none" />
                  )}
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6 w-full h-full">
                    <div className="flex flex-col flex-1 justify-between gap-3 h-full">
                      <div className="flex items-center gap-3" style={{ marginBottom: "-9px" }}>
                        <div className={`p-2 rounded-xl border flex-shrink-0 flex items-center justify-center transition-all ${
                          isExpanded
                            ? "bg-orange-500/10 border-orange-500/30 text-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.3)]"
                            : "bg-neutral-900 border-white/5 text-slate-500 group-hover:text-orange-500"
                        }`}>
                        ...
                        <div className="flex flex-col">
                          <span className={`font-black text-[19px] tracking-wider uppercase transition-colors ${
                            ...
                            MUSIC THEORY ANALYSIS V2 TESTING ELEMENT
                          </span>
                          <span className="text-[10px] text-slate-500 font-medium">Real Computed Data, Not Gemini Estimation</span>
                        </div>
                      </div>

                      <div className={`border-t text-left pt-2 px-0.5 transition-colors ${
                        isExpanded ? "border-orange-500/15" : "border-white/5"
                      }`}>
                        <p className="text-[10px] text-slate-400 leading-relaxed font-semibold" style={{ marginBottom: "0px" }}>
                          Test version — Chord Dynamics and Melody are computed directly from detected audio data (real chord recognition, real pitch transcription) instead of a single blended Gemini estimate.
                          <span className="block mt-1 text-orange-500/90 font-mono text-[8.5px] uppercase tracking-wider" style={{ marginBottom: "2px" }}>Testing card — compare against Music Theory Analysis above.</span>
                        </p>
                        <span
                          style={{ paddingTop: "2px" }}
                          className={`inline-block text-[9px] font-mono tracking-widest px-2 py-0.5 rounded-full border transition-all ${
                          isExpanded
                            ...
                            : "bg-neutral-900/50 border-white/5 text-slate-600 group-hover:text-orange-500"
                        }`}>
                          {isExpanded ? "ACTIVE ⬇" : "VIEW METRICS ⚡"}
                        </span>
                      </div>
                    ...
                    <div className="flex-shrink-0 flex items-center justify-center">
                      {v2OverallScore !== null ? (
                        <ScoreCircle
                          score={v2OverallScore}
                          size={110}
                          strokeWidth={7}
                          color={isExpanded ? "#f97316" : "rgba(249, 115, 22, 0.45)"}
                          glowColor={isExpanded ? "rgba(249, 115, 22, 0.65)" : "rgba(249, 115, 22, 0.15)"}
                          extraGlow={isExpanded}
                        />
                      ) : (
                        <div className="flex items-center justify-center w-[110px] h-[110px] rounded-full border border-dashed border-orange-500/30">
                ...
                <AnimatePresence initial={false}>
                  {isExpanded && (
                    <motion.div
                      ...
                      className="overflow-hidden w-full relative z-0"
                    >
                      <div style={{ position: "relative", left: "15px", width: "calc(100% - 15px)" }} className="bg-[#0A0B0E] border border-orange-500 rounded-3xl p-6 shadow-[0_0_35px_rgba(0,0,0,0.95)] flex flex-col gap-5">
                        <div style={{ fontFamily: "Inter, sans-serif", fontWeight: "bold", color: "#ffffff", fontSize: "16px" }}>
                          MUSIC THEORY ANALYSIS V2 AUDIT
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                          <div className="p-4 rounded-xl border bg-[#0d0d14] border-orange-500/15">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-300">Chord Dynamics</span>
                              <span className="text-[9px] font-mono font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full">REAL DATA</span>
                            </div>
                            <div className="text-xl font-black text-white font-mono mb-1.5">{v2ChordDynamicsScore !== null ? `${v2ChordDynamicsScore}` : "—"}</div>
                            <p className="text-[10px] text-slate-400 leading-relaxed">{v2ChordDynamicsDetail}</p>
                          </div>

                          <div className="p-4 rounded-xl border bg-[#0d0d14] border-orange-500/15">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-300">Melody</span>
                              <span className="text-[9px] font-mono font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full">REAL DATA</span>
                            </div>
                            <div className="text-xl font-black text-white font-mono mb-1.5">{v2MelodyScore !== null ? `${v2MelodyScore}` : "—"}</div>
                            <p className="text-[10px] text-slate-400 leading-relaxed">{v2MelodyDetail}</p>
                          </div>

                          <div className="p-4 rounded-xl border bg-[#0d0d14] border-amber-500/15">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-300">Form & Structure</span>
                              <span className="text-[9px] font-mono font-bold text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">GEMINI EST.</span>
                            </div>
                            <div className="text-xl font-black text-white font-mono mb-1.5">{v2FormScore}</div>
                            <p className="text-[10px] text-slate-400 leading-relaxed">No real Structural Boundary Detection data exists yet — this still reflects Gemini's holistic estimate, same as the V1 card above.</p>
                          </div>
                        </div>

                        <p className="text-[10px] text-slate-500 leading-relaxed border-t border-white/5 pt-3">
                          This card is a live test of replacing Gemini's single blended Music Theory score with real, independently computed sub-scores. If this proves reliable, it can replace the card above; if not, it can be removed without affecting anything else.
                        </p>
                      </div>
                    </motion.div>
                  )}
