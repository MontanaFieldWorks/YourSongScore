...
THE CALIBRATION ANCHOR: a competent, professionally mixed and mastered commercial release - the standard of a major-label single that charted respectably - sits at 82-88 on any given metric. That is the CENTRE of this scale, not the bottom of it. It describes work that is genuinely professional and has no real flaw. A score in the 80s is a good, respectable, professional-standard result and must never be written about as though it were a failure or a disappointment.
...
ANTI-CLUSTERING - MANDATORY: do not converge on a habitual value. In real measured output from this system, the single value 92 was assigned to nearly 17% of all scored fields across many completely different tracks, and five values covered over half of every score given. That is a symptom of defaulting to a comfortable number, not of genuine measurement. Two different metrics on the same track, or the same metric on two very different tracks, should rarely land on the identical number unless the underlying evidence is genuinely identical. Use the full width of each band, including values ending in 1, 3, 6 and 7 - 83, 86, 87 and 91 are exactly as legitimate as 90 or 95, and a set of scores clustered on one number or mostly landing on multiples of 5 is a failure of measurement rather than a coincidence.
...
GATE: if the measured value is low (below 45), before scoring below 85, explicitly check: does the centered arrangement fit the genre's convention and keep the mix feeling intentional rather than accidentally flat or undeveloped? If YES - name that genre fit in the commentary and score 85-100. If NO - meaning the lack of stereo movement reads as an underdeveloped mix rather than a stylistic choice - score using the bands below.
RUBRIC ANCHOR: measured 75-100 -> score 85-100 (genuinely wide, deliberate spatial placement - a mix using the full stereo field as part of its arrangement, the kind of production regularly cited as pioneering for its use of panning). Measured 45-74 -> score 65-84 (some real stereo movement and placement, not purely centered but not maximally wide either). Measured 20-44 -> score 45-64 only if the gate above finds the centered arrangement genuinely undeveloped - otherwise 85-100 per the gate. Below 20 -> score below 45 only if the gate above finds the centered arrangement genuinely undeveloped - otherwise 85-100 per the gate.
...
GATE: if the measured value is low (below 55), before scoring below 90, explicitly check: does the vocal show genuine register shifts, phrasing variation, or emotional escalation across sections despite the flat raw loudness (e.g. an intimate low verse building to a soaring falsetto or belted chorus)? If YES - name that specific register/phrasing evidence in the commentary and score 90-100; the low raw-loudness measurement does not override real, audible expressive variety. If NO - meaning the vocal is genuinely flat in register, phrasing, AND loudness with no describable expressive arc - score using the bands below.
RUBRIC ANCHOR: measured 80-100 -> score 85-100 (genuine, wide dynamic range, e.g. a whisper-to-belt vocal arc); measured 55-79 -> score 65-84 (real but moderate push and pull); measured 30-54 -> score 45-64 only if the gate above finds no genuine register/phrasing variety - otherwise 90-100 per the gate; below 30 -> score below 45 only if the gate above finds no genuine register/phrasing variety - otherwise 90-100 per the gate.
