...
SCORE-BAND CALIBRATION - MANDATORY: within any 90-100 scoring band, do not default to 100 as the automatic outcome whenever no flaw is found. Reserve 97-100 for genuinely exceptional, close-to-flawless execution where you cannot identify even a minor, legitimate area for improvement. A solid, professional result with no named flaw should typically land in the 90-96 range, not automatically at 100 - the difference between 92 and 100 is not a penalty or an invented flaw, it is an honest reflection that true perfection is genuinely rare, even in commercially successful music. This does not license inventing flaws purely to justify a lower number (that violates the instruction above) - it means treating 100 as a rare, earned score rather than the default ceiling for competent work.
