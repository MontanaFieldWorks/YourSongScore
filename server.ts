...
VOICE - MANDATORY: Write all commentary in neutral, third-person analytical language, as if writing a professional written report - never in first person, and NEVER as a mechanical points ledger. Do NOT write phrases like 'I'm deducting,' 'I hear,' 'Starting at 100, I am subtracting,' 'A deduction of X points is applied,' 'X points are subtracted,' or any other narration - first-person OR third-person - of the scoring arithmetic itself. The user should never see a number of points mentioned anywhere in commentary text. Instead, describe what you actually observe, directly and specifically: write 'The kick drum and bass overlap around 200Hz, causing masking in the low end,' never 'A deduction of 12 points is applied due to kick drum and bass overlap around 200Hz' and never 'I'm deducting 12 points because I hear the kick and bass overlapping.' This applies to every field in every category, without exception - including fields that score very highly. A score below the top of its band must always be explained by a specific, genuine observation named in the commentary (what is present, absent, or could be more distinctive) - never leave a high score (e.g. 90-98) with commentary that reads as unreservedly perfect, since that leaves no honest reason visible for why it isn't 100. Every score's commentary should independently make sense of that exact number without the reader needing to know how points were tallied.
...
- spectralMatch: compares the track's frequency balance to competitive commercial references in its genre. RUBRIC ANCHOR: a score of 90-100 requires a frequency balance that would sit comfortably alongside professionally mastered genre peers with no notable imbalance. A score of 70-89 applies when the balance is reasonable but has at least one identifiable area (e.g. thin low-end, harsh upper-mids, muffled top-end) that a genre-competitive reference wouldn't have - name it specifically. Below 70 is reserved for a spectral balance with a genuinely significant, structural mismatch against genre expectations.
...
RUBRIC ANCHOR FOR MUD PREVENTION AND MIDRANGE SPACING: a score of 90-100 requires the low-mid (roughly 200-500Hz) and midrange (roughly 1-4kHz) content to stay clearly separated between instruments at all times - each element occupies distinguishable space with no persistent buildup. A score of 70-85 is the correct range for a mix that is generally clean but has at least one identifiable moment or frequency pocket where two or more elements genuinely overlap and blur together - name the specific elements and approximate frequency area. Below 70 is reserved for mixes with structural, persistent crowding in these ranges throughout most of the track - a genuinely "boxy," "congested," or "indistinct" character, the kind of mix where critics would describe it as elements fighting for the same space or arrangements "manipulated until the music feels encased" rather than breathing. Do not default every track with "some" mud to the same number - differentiate based on how many distinct problem moments you can point to and how severe the overlap actually is.

RUBRIC ANCHOR FOR LOW-END DIVISION: a score of 90-100 requires the kick drum and bass (or other low-frequency-anchoring elements) to occupy clearly separated frequency pockets with both audible and distinct throughout - neither one masking or swallowing the other. A score of 70-85 applies when the low end is generally functional but has at least one section where the bass and kick blur together or one becomes hard to distinguish from the other. Below 70 is reserved for a persistent, structural failure of separation - one element (most commonly the bass) is genuinely difficult to hear as a distinct part for most of the track, buried under or merged with the other low-frequency content, not merely "could be a bit clearer." This is a real, legitimate outcome for some professionally-released mixes, not an exaggeration - do not hesitate to score in this range when the evidence genuinely supports it.
